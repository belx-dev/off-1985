...
| `Evaluator/` | header-only | `engine/Evaluator/CLAUDE.md` | SQF expression evaluator core (parser + `GameValue`/`GameState`); consumed by `Poseidon/Game/Scripting`. |
| `Random/` | header-only | `engine/Random/CLAUDE.md` | Deterministic seeded PRNG for world/NPC generation. |
